...
class ArrayDecoderTest extends WordSpec with Matchers {
  ...
  "Decoder" should {
    ...
    "support array for a vector of primitives" in {
      ...
      Decoder[TestVectorBooleans].decode(record, schema, DefaultFieldMapper) shouldBe TestVectorBooleans(Vector(true, false, true)).asRight
    }
    ...
    "support array for an vector of records" in {
      ...
      Decoder[TestVectorRecords].decode(container, containerSchema, DefaultFieldMapper) shouldBe TestVectorRecords(Vector(Record("qwe", 123.4), Record("wer", 8234.324))).asRight
    }
    ...
    "support array for an Array of primitives" in {
      ...
      val record = new GenericData.Record(schema)
      record.put("booleans", List(true, false, true).asJava)
      Decoder[TestArrayBooleans].decode(record, schema, DefaultFieldMapper).map(_.booleans.toVector) shouldBe Vector(true, false, true).asRight
    }
    ...
    "support array for a List of primitives" in {
      ...
      val record = new GenericData.Record(schema)
      record.put("booleans", List(true, false, true).asJava)
      Decoder[TestListBooleans].decode(record, schema, DefaultFieldMapper) shouldBe TestListBooleans(List(true, false, true)).asRight
    }
    ...
    "support array for a List of records" in {
      ...
      Decoder[TestListRecords].decode(container, containerSchema, DefaultFieldMapper) shouldBe TestListRecords(List(Record("qwe", 123.4), Record("wer", 8234.324))).asRight
    }
    ...
    "support array for a scala.collection.immutable.Seq of records" in {
      ...
      Decoder[TestSeqRecords].decode(container, containerSchema, DefaultFieldMapper) shouldBe TestSeqRecords(Seq(Record("qwe", 123.4), Record("wer", 8234.324))).asRight
    }
    ...
    "support array for an Array of records" in {
      ...
      Decoder[TestArrayRecords].decode(container, containerSchema, DefaultFieldMapper).map(_.records.toVector) shouldBe Vector(Record("qwe", 123.4), Record("wer", 8234.324)).asRight
    }
    ...
    "support array for a Set of records" in {
      ...
      Decoder[TestSetRecords].decode(container, containerSchema, DefaultFieldMapper) shouldBe TestSetRecords(Set(Record("qwe", 123.4), Record("wer", 8234.324))).asRight
    }
    "support array for a Set of strings" in {
      val schema = AvroSchema[TestSetString]
      val record = new GenericData.Record(schema)
      record.put("strings", List("Qwe", "324", "q").asJava)
      Decoder[TestSetString].decode(record, schema, DefaultFieldMapper) shouldBe TestSetString(Set("Qwe", "324", "q")).asRight
    }
    "support array for a Set of doubles" in {
      ...
      Decoder[TestSetDoubles].decode(record, schema, DefaultFieldMapper) shouldBe TestSetDoubles(Set(132.4324, 5.4, 0.123)).asRight
    }
    ...
    "support top level Seq[Double]" in {
      Decoder[Seq[Double]].decode(Array(1.2, 34.5, 54.3), AvroSchema[Seq[Double]], DefaultFieldMapper) shouldBe Seq(1.2, 34.5, 54.3).asRight
    }
    "support top level List[Int]" in {
      Decoder[List[Int]].decode(Array(1, 4, 9), AvroSchema[Seq[Int]], DefaultFieldMapper) shouldBe List(1, 4, 9).asRight
    }
    "support top level Vector[String]" in {
      Decoder[Vector[String]].decode(Array("a", "z"), AvroSchema[Seq[String]], DefaultFieldMapper) shouldBe Vector("a", "z").asRight
    }
    "support top level Set[Boolean]" in {
      Decoder[Set[Boolean]].decode(Array(true, false, true), AvroSchema[Seq[Boolean]], DefaultFieldMapper) shouldBe Set(true, false).asRight
    }
  }
}